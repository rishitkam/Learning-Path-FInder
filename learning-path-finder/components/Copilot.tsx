...
export default function Copilot({ data, onPath }: { data: PathData | null; onPath: (data: PathData) => void }) {
  ...
  async function send(event: FormEvent) { event.preventDefault(); const message = input.trim(); if (!message || busy) return; setInput(""); setMessages((current) => [...current, { role: "user", content: message }]); setBusy(true); try { const activeProfile = data?.profile ?? profile; const result = await chat(message, activeProfile, data?.state.completed ?? [], data?.state.blocked ?? []); if (result.profile) setProfile({ ...defaultProfile, ...result.profile }); if (result.data) { setProfile(result.data.profile); onPath(result.data); } setMessages((current) => [...current, { role: "assistant", content: result.reply }]); } catch (reason) { setMessages((current) => [...current, { role: "assistant", content: reason instanceof Error ? reason.message : "I couldn’t reach the co-pilot." }]); } finally { setBusy(false); } }
  return <aside className="copilot"><div className="copilot-head"><div className="copilot-icon"><Bot size={17}/></div><div><p>ALMA CO-PILOT</p><small><i/> LLM connected</small></div></div><div className="chat-thread">{messages.map((message, index) => <div className={`chat-message ${message.role}`} key={index}>{message.role === "assistant" && <Sparkles size={13}/>}<span>{message.content}</span></div>)}{busy && <div className="chat-typing"><i/><i/><i/></div>}</div><form className="chat-form" onSubmit={send}><input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Tell ALMA your goal…"/><button aria-label="Send message" disabled={busy || !input.trim()}><Send size={15}/></button></form></aside>;
}
