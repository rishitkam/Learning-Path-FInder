...
export default function Home() {
  ...
  return <div className="app-shell"><Sidebar/><div className="app-content"><Header/><main className="dashboard">
    ...
    <section className="dashboard-grid"><div className="path-card"><div className="card-heading"><div><p className="section-kicker">YOUR LEARNING CONSTELLATION</p><h2>{view === "roadmap" ? "The big picture" : "The next steps"}</h2></div><div className="path-legend"><span><i className="done"/> Mastered</span><span><i className="current"/> In progress</span></div></div><RoadmapGraph view={view} path={data?.path} completed={data?.state.completed ?? []} loading={loading}/></div><div className="side-stack"><TelemetryCards data={data} loading={loading} error={error} onFeedback={feedback}/></div></section>
